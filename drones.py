...
class DroneStore(object):
    ...
    def save(self, drone):
        ...
        query = "INSERT INTO drone_table(name, class_type, rescue_type, operator_ID) \
        VALUES{'" + str(drone.name) + "', " + str(drone.class_type) + ", " + str(drone.rescue) + ", " + str(drone.rescue) + ");"
        dbcursor = self._conn.cursor(buffered=True)
